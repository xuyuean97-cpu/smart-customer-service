...
async def send_order_info_to_user(sql_result):
    ...
    if len(order_details) > 0:
        ...
        for order in order_details:
            ...
            tmp["action_supported"] = True 
            
            # 注入Logo
            exp_code = get_express_code_by_id(str(tmp.get('order_id')))
            tmp["express_logo"] = logo_data.get(exp_code)
            
            send_data_list.append(tmp)
...
async def order_logistics_agent(state: EcommerceMainServiceState, config: RunnableConfig):
    ...
    if sql_result:
        try:
            ...
            pass
    print(f"DEBUG RESPONSE s##4444skkk: {res}")
...
async def order_logistics_search(state: EcommerceMainServiceState, config: RunnableConfig):
    ...
    # 调用电商版的检索函数
    retrieval_result = await order_query2docs(user_query,user_id,messages)
