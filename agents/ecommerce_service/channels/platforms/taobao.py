"""
淘宝/天猫开放平台适配器 — 千牛消息 + 订单/物流查询
"""
import hashlib
import time
import json
from typing import Optional
import httpx

from . import EcommercePlatformAdapter, register_adapter
from .. import ChannelMessage
from models.platform_order import UnifiedOrder, UnifiedLogistics, PlatformCredential
from common.logging import get_logger

logger = get_logger("channels.platforms.taobao")

TAOBAO_API_BASE = "https://eco.taobao.com/router/rest"
TAOBAO_OAUTH_URL = "https://oauth.taobao.com/authorize"


class TaobaoAdapter(EcommercePlatformAdapter):
    platform = "taobao"

    def __init__(self, credential: Optional[PlatformCredential] = None):
        super().__init__(credential)

    # ===== 签名验证 =====

    async def verify_signature(self, **kwargs) -> bool:
        """淘宝回调 MD5 签名验证"""
        sign = kwargs.get("sign", "")
        params = {k: v for k, v in kwargs.items() if k not in ("sign", "sign_method")}
        sorted_params = sorted(params.items())
        raw = self.credential.app_secret + "".join(f"{k}{v}" for k, v in sorted_params) + self.credential.app_secret
        expected = hashlib.md5(raw.encode()).hexdigest().upper()
        return sign.upper() == expected

    # ===== 消息解析 =====

    async def parse_message(self, raw_data: dict) -> ChannelMessage:
        """千牛消息 JSON → ChannelMessage"""
        msg_type = raw_data.get("type", "text")
        from_user = raw_data.get("buyer_nick", raw_data.get("user_id", "unknown"))
        content = raw_data.get("content", raw_data.get("text", ""))

        if msg_type == "image":
            content = raw_data.get("pic_url", "[图片消息]")

        return ChannelMessage(
            query=content,
            user_id=f"taobao_{from_user}",
            tenant_id="default",
            channel="taobao",
            metadata={
                "platform": "taobao",
                "msg_type": msg_type,
                "from_user_id": from_user,
                "consumer_name": raw_data.get("buyer_nick", f"淘宝用户"),
                "raw": raw_data,
            },
        )

    async def build_reply(self, response_text: str, original_msg: ChannelMessage) -> dict:
        """构建千牛回复格式"""
        return {
            "code": 0,
            "msg": "success",
            "data": {
                "toUser": original_msg.metadata.get("from_user_id", ""),
                "content": response_text,
                "msgtype": "text",
            },
        }

    # ===== 订单查询 =====

    async def get_order(self, order_id: str) -> Optional[UnifiedOrder]:
        """调用淘宝 API 查询单个订单"""
        result = await self._call_api("taobao.trade.get", {
            "tid": order_id,
            "fields": "tid,buyer_nick,payment,status,created,pay_time,consign_time,shipping_type,receiver_name,receiver_address,receiver_mobile",
        })
        if not result:
            return None
        try:
            data = result.get("trade_get_response", {}).get("trade", {})
            return UnifiedOrder(
                platform="taobao",
                platform_order_id=str(data.get("tid", order_id)),
                product_name=data.get("title", ""),
                total_amount=float(data.get("payment", 0)),
                order_status=self._map_status(data.get("status", "")),
                buyer_nick=data.get("buyer_nick", ""),
                recipient_name_masked=data.get("receiver_name", ""),
                phone_encrypted=data.get("receiver_mobile", ""),
                address_masked=data.get("receiver_address", ""),
                created_at=data.get("created"),
                paid_at=data.get("pay_time"),
                shipped_at=data.get("consign_time"),
            )
        except Exception as e:
            logger.warning(f"解析淘宝订单失败: {e}")
            return None

    async def get_logistics(self, tracking_no: str) -> Optional[UnifiedLogistics]:
        """查询淘宝物流轨迹"""
        result = await self._call_api("taobao.logistics.trace.search", {
            "tid": tracking_no,
            "fields": "tid,seller_id,company_name,out_sid,status,created",
        })
        if not result:
            return None
        try:
            data = result.get("logistics_trace_search_response", {}).get("trace_list", {})
            traces = data.get("transit_step_info_list", [])
            return UnifiedLogistics(
                tracking_number=tracking_no,
                express_company=data.get("company_name", "未知快递"),
                status=self._map_logistics_status(data.get("status", "")),
                details=[
                    {"time": t.get("time", ""), "location": t.get("desc", ""), "description": t.get("desc", "")}
                    for t in traces
                ],
            )
        except Exception as e:
            logger.warning(f"解析淘宝物流失败: {e}")
            return None

    # ===== 消息发送 =====

    async def send_message(self, user_id: str, text: str) -> bool:
        """调用千牛发送消息"""
        result = await self._call_api("taobao.miniapp.message.send", {
            "to_user": user_id,
            "content": text,
        })
        return result is not None

    # ===== Token 管理 =====

    async def refresh_token(self) -> PlatformCredential:
        result = await self._call_api("taobao.auth.token.refresh", {
            "refresh_token": self.credential.refresh_token,
        })
        if result:
            data = result.get("auth_token_refresh_response", {}).get("token_result", {})
            self.credential.access_token = data.get("access_token", self.credential.access_token)
            self.credential.refresh_token = data.get("refresh_token", self.credential.refresh_token)
            self.credential.expires_at = int(time.time()) + data.get("expires_in", 86400)
        return self.credential

    # ===== 内部方法 =====

    async def _call_api(self, method: str, params: dict) -> Optional[dict]:
        """通用淘宝 API 调用（HMAC-MD5 签名）"""
        if not self.credential:
            logger.warning("淘宝适配器未配置凭据")
            return None
        try:
            sys_params = {
                "method": method,
                "app_key": self.credential.app_key,
                "session": self.credential.access_token,
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "format": "json",
                "v": "2.0",
                "sign_method": "hmac",
            }
            all_params = {**sys_params, **{k: str(v) for k, v in params.items()}}
            sorted_params = sorted(all_params.items())
            sign_str = self.credential.app_secret + "".join(f"{k}{v}" for k, v in sorted_params) + self.credential.app_secret
            all_params["sign"] = hashlib.md5(sign_str.encode()).hexdigest().upper()

            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.post(TAOBAO_API_BASE, data=all_params)
                if resp.status_code == 200:
                    return resp.json()
                logger.error(f"淘宝 API 返回 {resp.status_code}: {resp.text[:200]}")
                return None
        except Exception as e:
            logger.error(f"淘宝 API 调用失败 [{method}]: {e}")
            return None

    def _map_status(self, tb_status: str) -> str:
        mapping = {
            "WAIT_BUYER_PAY": "pending",
            "WAIT_SELLER_SEND_GOODS": "paid",
            "WAIT_BUYER_CONFIRM_GOODS": "shipped",
            "TRADE_FINISHED": "delivered",
            "TRADE_CLOSED": "cancelled",
        }
        return mapping.get(tb_status, "unknown")

    def _map_logistics_status(self, status: str) -> str:
        mapping = {
            "NOT_SEND": "未发货",
            "SEND": "已发货",
            "CONFIRM_RECEIPTS": "已签收",
            "CLOSED": "已关闭",
        }
        return mapping.get(status, "运输中")


# 自动注册
register_adapter("taobao", TaobaoAdapter())
