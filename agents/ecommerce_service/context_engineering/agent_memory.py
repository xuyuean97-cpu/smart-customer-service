    ...
    async def store_agent_conversation_interaction(
        ...
        try:
            ...
            if isinstance(messages, list):
                safe_query = "\n".join([get_text(m) for m in messages[-2:]])
    ...
    def decorator(agent_func):
        async def wrapper(state: Dict[str, Any], config=None, *args, **kwargs):
            ...
            try:
                ...
                # 6. 异步写入 LLM Token 用量
                try:
                    from agents.ecommerce_service.core.usage_tracker import usage_tracker
                    cb = usage_tracker.get_callback(tenant_id=tenant_id)
...
class MemoryEnabledAgent:
    ...
    async def store_interaction(
        ...
        """存储交互记忆"""
        return await AgentMemoryMixin.store_agent_conversation_interaction(
            user_id=user_id,
    ...
    async def get_relevant_memories(
        ...
        """获取相关记忆"""
        return await AgentMemoryMixin.retrieve_relevant_conversation_memories(
            query=current_query,
            user_id=user_id,
            agent_id=self.agent_id,
            limit=limit
    ...
    async def get_smart_filtered_memories(
        ...
        """获取智能筛选的记忆"""
        from .memory_manager import memory_manager
        return await memory_manager.get_smart_filtered_memories(
            user_id=user_id,
            query=current_query,
            agent_id=self.agent_id,
            limit=limit,
