...
class BehaviorAggregator:
    ...
    def _calculate_comprehensive_behavior_score(self, daily_profiles: List[DailyProfile]) -> Dict[str, float]:
        ...
        # 2. 解决率趋势
        resolution_scores = [p.behavior_pattern.resolution_rate for p in daily_profiles]
        resolution_trend = self._calculate_trend(resolution_scores)

        # 3. 参与深度稳定性
        engagement_scores = [p.interaction_metrics.total_turns for p in daily_profiles]
        engagement_stability = 1.0 - (statistics.stdev(engagement_scores) / (statistics.mean(engagement_scores) + 0.1))

        return {
            "activity_consistency": max(0, min(activity_consistency, 1.0)),
            "resolution_trend": resolution_trend,
            "engagement_stability": max(0, min(engagement_stability, 1.0)),
            "overall_stability": (activity_consistency + engagement_stability) / 2
        }
