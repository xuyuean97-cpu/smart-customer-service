...
async def order_rewrite_query(query: str, messages: List[AnyMessage]) -> str:
    """
    专门针对订单/物流场景的重写器。
    逻辑：从对话历史中抓取订单号、快递单号或商品指代。
    """
    prompt = """你是一个电商意图补全专家。请结合对话历史，将用户的问题改写为一个包含具体实体的完整查询语句。
    
    示例：
    历史：用户问“我的订单 12345 什么时候发货？”，客服答“正在备货”。
    当前问题：“到哪了？”
    改写结果：“查询订单 12345 的物流当前轨迹位置”
    
    历史：无
    当前问题：“我的快递呢？”
    改写结果：“查询用户最近一笔订单的物流状态”
    
    请输出改写后的句子。"""
    
    # 这里调用你的 LLM chain
    # res = await rewrite_chain.ainvoke({"query": query, "messages": messages, "instruction": prompt})
    # return res.content
    pass